...
import type { AnthropicConfig, GoogleConfig, OpenAIConfig, XAIConfig } from "./configs.js";

// Re-export model types and data
export * from "./generated/models.js";

// Provider utilities for CLI usage
export function getProviderForModel(model: AllModels): "anthropic" | "openai" | "google" | "xai" {
	return ModelToProvider[model as keyof typeof ModelToProvider];
}

export function getDefaultApiKeyEnvVar(provider: "anthropic" | "openai" | "google" | "xai"): string {
	switch (provider) {
		case "anthropic":
...
export function createClientForModel(
	model: AllModels,
	config: AnthropicConfig | OpenAIConfig | GoogleConfig | XAIConfig,
): ChatClient {
	...
	if (provider === "anthropic") {
		...
		return lemmy.xai({ ...config, model } as XAIConfig);
	} else {
...
export function findModelData(model: string): ModelData | undefined {
	...
	if (XAIModelData[model as keyof typeof XAIModelData]) {
		return XAIModelData[model as keyof typeof XAIModelData] as ModelData;
	}
	return undefined;
}
